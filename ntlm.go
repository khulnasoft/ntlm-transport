...
func init() {
	kengine.RegisterModule(NTLMTransport{})
}
...
func (NTLMTransport) KengineModule() kengine.ModuleInfo {
...
func (n *NTLMTransport) Provision(ctx kengine.Context) error {
	n.ctx = ctx
	n.transports = make(map[string]*http.Transport)
	n.transportsMu = new(sync.RWMutex)

	if n.HTTPTransport == nil {
		n.HTTPTransport = new(reverseproxy.HTTPTransport)
	}
...
func (n *NTLMTransport) newTransport() (*http.Transport, error) {
	...
	transport.DialContext = func(ctx context.Context, network, address string) (net.Conn, error) {
		...
		req := ctx.Value(kenginehttp.OriginalRequestCtxKey).(http.Request)
...
func (n NTLMTransport) TLSEnabled() bool {
	return n.HTTPTransport.TLS != nil
}

// EnableTLS enables TLS on the transport.
func (n *NTLMTransport) EnableTLS(base *reverseproxy.TLSConfig) error {
	n.HTTPTransport.TLS = base
	return nil
}

// Cleanup implements kengine.CleanerUpper and closes any idle connections.
...
// Interface guards
var (
	_ kengine.Provisioner         = (*NTLMTransport)(nil)
	_ http.RoundTripper         = (*NTLMTransport)(nil)
	_ kengine.CleanerUpper        = (*NTLMTransport)(nil)
	_ reverseproxy.TLSTransport = (*NTLMTransport)(nil)
)
